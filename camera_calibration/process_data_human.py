...
# Add the arguments
parser.add_argument(
    "--data_dir", required=False, type=str, help="Path to the data directory"
)
parser.add_argument("--task_names", nargs="+", type=str, help="List of task names")
parser.add_argument(
    "--process_depth", action="store_true", help="Enable processing depth data"
)
parser.add_argument(
    "--num_demos", type=int, default=None, help="Number of demonstrations to process"
)

parser.add_argument("--camera", type=int, nargs="+", default=[3, 4], help="Camera ID")

args = parser.parse_args()
args.data_dir = "/home/david/egozero/Franka-Teach/data"
    ...
    for num, demo_dir in enumerate(sorted(DEMO_DIRS)):
        ...
        if process_sensor:
            ...
            for i in range(len(sensor_values)):
                ...
                sensor_history_test.append(sensor_history[i].flatten())
            sensor_values_test = pd.DataFrame(
                {"column": [list(row) for row in sensor_values_test]}
            )
            sensor_history_test = pd.DataFrame(
                {"column": [list(row) for row in sensor_history_test]}
            )

            sensor_test = pd.concat(
                [sensor_timestamps_test, sensor_values_test, sensor_history_test],
                axis=1,
            )
