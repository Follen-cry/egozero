...
class RGBArrayAsObservationWrapper(dm_env.Environment):
    ...
    def __init__(
        ...
        task_name,
        object_labels,
        root_dir,  # for expert labeled keypoints and image, should have preprocess/label_keypoints.* inside it
        calib_path,
        width=256,
        height=256,
        use_robot=False,
        ...
        use_gt_depth=False,
        point_dim=3,
        prompts=[],  # prompts for grounded dino segmentation, for better dift
    ):
        """
    ...
    def reset(self, **kwargs):
        ...
        robot_points, robot_points_3d = None, None
        ...
        if self._point_dim == 2:
            ...
            if not self._use_gt_depth:
                ...
                for pixel_key in self._pixel_keys:
                    ...
                    observation[f"point_tracks_{pixel_key}"] = (pt2d, depths)
    ...
    def render(self, mode="rgb_array", cam_idx=None, width=256, height=256):
        return cv2.cvtColor(
            self._env.render("rgb_array", cam_idx=cam_idx), cv2.COLOR_BGR2RGB
        )
