...
from agent.networks.rgb_modules import DinoV2Encoder, ResnetEncoder
...
class BCAgent:
    def __init__(
        ...
        print(
            "mapping 3d pos + 3d rot + 1d grasp -> 3d pos + 1d grasp action dimension"
        )
        ...
        self.point_encoder = nn.Linear(3, self.repr_dim).to(self.device)
        ...
        # optimizers
        # encoder
        params = list(self.point_encoder.parameters())
        if self.use_depth:
    ...
    def train(self, training=True):
        ...
        if training:
            ...
            self.point_encoder.train(training)
            if self.use_proprio:
    ...
    def act(self, obs, norm_stats, step, global_step, eval_mode=False):
        ...
        for key in self.pixel_keys:
            features.append(
                self.point_encoder(
                    torch.tensor(obs[key], device=self.device, dtype=torch.float32)
                )
            )
        ...
        return action.cpu().numpy()[
            0, -1, :
        ]  # NOTE: usually returns last action but return all for open loop
    ...
    def update(self, expert_replay_iter, step):
        ...
        for key in self.pixel_keys:
            features.append(
                self.point_encoder(
                    torch.tensor(data[key], device=self.device, dtype=torch.float32)
                )
            )
