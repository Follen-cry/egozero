...
class BCAgent:
    def __init__(
        ...
        # observation params
        self._obs_dim = point_dim * (
            self.num_track_points + 2 * bool(self.history)
        )  # if history, then it's closed loop which means we pass in proprio as +1 channel
        self.repr_dim = 512
        obs_shape = obs_shape[self.pixel_keys[0]]

        # actor parameters
        # Action dim was 7 with 3D position + 4D rotation + gripper
        # Now it is 4D with 3D position + gripper
        ...
        # actor
        action_dim = (
            self._act_dim * self.num_queries
            if self.temporal_agg or not self.history
            else self._act_dim
        )
        self.actor = Actor(
            self.repr_dim,
    ...
    def act(self, obs, norm_stats, step, global_step, eval_mode=False, **kwargs):
        ...
        for key in self.pixel_keys:
            if norm_stats is not None:
                ...
                point_tracks = obs[f"point_tracks_{key}"]
